...
public class Sem4 {
  ...
  public static void main(String[] args) {
    ...
    for (int j = 0; j < index.size(); j++) {
      System.out.printf(
        "%s %s. %s. %s %s \n",
        familia.get(index.get(j)),
        name.get(index.get(j)).charAt(0),
        batka.get(index.get(j)).charAt(0),
        age.get(index.get(j)),
        gender.get(index.get(j))
      );
    }
    ...
    for (int j = 0; j < index.size() - 1; j++) {
      if (
        temporary2.get(j).contains("м") && temporary2.get(j + 1).contains("ж")
      ) {
